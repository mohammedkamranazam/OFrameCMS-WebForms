...
{
    public partial class SizesAdd : System.Web.UI.Page
    {
        ...
        private void FormToolbar1_Save(object sender, EventArgs e)
        {
            if (IsValid)
            {
                using (var context = new ShoppingCartEntities())
                {
                    ...
                    {
                        var sizeEntity = new SC_Sizes();
                        sizeEntity.Title = TitleTextBox.Text;
                        sizeEntity.Description = DescriptionTextBox.Text;
                        sizeEntity.SectionID = (int)SectionsDropDownList.GetNullableSelectedValue();
                        sizeEntity.CategoryID = (int)CategoriesDropDownList.GetNullableSelectedValue();
                        sizeEntity.SubCategoryID = (int)SubCategoriesDropDownList.GetNullableSelectedValue();

                        try
                        {
                            SizesBL.Add(sizeEntity, context);
